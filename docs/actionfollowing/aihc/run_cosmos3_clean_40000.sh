...
BOOTSTRAP_LOG_DIR=/mnt/dataset/csx_ckp/Action-Following/outputs/cosmos3/clean/bootstrap_logs
...
# ACWM-Motus_clean_40000 runs.  Cosmos keeps its own current1+future32
...
OUT_BASE="/mnt/gyc_ckp/Action-Following/outputs/cosmos3/clean/train_40000_20260721_motusdata_future32_prompt_${RUN_TAG}"
...
mkdir -p "$HF_DATASETS_CACHE" "$XDG_CACHE_HOME" "$TMPDIR"
{
  ...
  sha256sum "$REPO/cosmos_framework/data/generator/action/datasets/actionfollowing_lerobot_dataset.py" \
    ...
    "$REPO/examples/toml/sft_config/actionfollowing_full50_clean.toml" \
    "$REPO/examples/launch_sft_actionfollowing_full50_clean.sh" \
    "$0"
...
echo "[CONTRACT] optimizer_steps=40000 checkpoint_save_iter=5000 scheduler_cycle=40000 warmup_steps=1000"
...
DRYRUN_JOB_NAME="cosmos3_nano_afd_full50_clean_rot6d20_a32_future32_prompt_bs16_40000step_dryrun"
AFD_PROTOCOL=clean \
BASE_CHECKPOINT_PATH="$BASE_CHECKPOINT_PATH" \
WAN_VAE_PATH="$WAN_VAE_PATH" \
"$REPO/.venv/bin/python" -m cosmos_framework.scripts.train \
  --sft-toml=examples/toml/sft_config/actionfollowing_full50_clean.toml \
  --dryrun \
  -- \
...
run_train() {
  ...
  local run_name="cosmos3_nano_afd_full50_clean_rot6d20_a32_future32_prompt_${label}_40000step"
  mkdir -p "$output_root"
  echo "[TRAIN] label=$label per_rank_batch=$per_rank_batch global_batch=$((per_rank_batch * 8)) optimizer_steps=40000 checkpoint_save_iter=5000"
  AFD_PROTOCOL=clean \
  BASE_CHECKPOINT_PATH="$BASE_CHECKPOINT_PATH" \
  WAN_VAE_PATH="$WAN_VAE_PATH" \
  OUTPUT_ROOT="$output_root" \
  IMAGINAIRE_OUTPUT_ROOT="$output_root" \
  EXTRA_TAIL_OVERRIDES="trainer.max_iter=40000 trainer.logging_iter=10 checkpoint.save_iter=5000 dataloader_train.max_samples_per_batch=$per_rank_batch scheduler.cycle_lengths=[40000] scheduler.warm_up_steps=[1000] job.name=$run_name" \
  MASTER_PORT=29541 \
  NPROC_PER_NODE=8 \
  bash "$REPO/examples/launch_sft_actionfollowing_full50_clean.sh"
}
...
TRAIN_LOG="$TRAIN_OUT/logs/actionfollowing_full50_clean_sft.log"
