...
Both canonical roots contain `DATA_AUDIT.log` (or the Predict2.5 equivalent data audit), `SMOKE_RESULT.txt`, `latest_checkpoint.txt`, and a real `iter_000000020` checkpoint. After separate user approval, the Cosmos3 `mix4` 40k bundle at PR commit `73e7fe7` was submitted as `job-3cmb7l4p44jw` to `cce-pmm1yohj/train21`; it entered `Running` on 2026-07-21 and saved a valid 137G `iter_000010000` checkpoint. This already-running job retains its launch-time 10000-step save interval. The Cosmos3 clean 40k bundle at PR commit `cc06da6` was submitted as `job-ogwrcxzuaokw` to the same queue; it passed real clean/mix4 audit, loaded the base DCP, entered real optimizer steps with global batch 16, and saves every 5000 optimizer steps. No 40k launch is complete until step 40000, finite losses, `iter_000040000`, latest marker, and `TRAIN_RESULT.txt` are all verified.
